...
EXTENSION(pg_curl_mime_file) {
    CURLcode res = CURL_LAST;
    char *filename, *name = NULL, *type = NULL, *code = NULL;
    curl_mimepart *part;
...
EXTENSION(pg_curl_mime_file_array) {
    CURLcode res = CURL_LAST;
    Datum *filename_elemsp, *name_elemsp = NULL, *type_elemsp = NULL, *code_elemsp = NULL;
    bool *filename_nullsp, *name_nullsp = NULL, *type_nullsp = NULL, *code_nullsp = NULL;
    int filename_nelemsp, name_nelemsp = 0, type_nelemsp = 0, code_nelemsp = 0;
    if (PG_ARGISNULL(0)) ereport(ERROR, (errmsg("filename is null!")));
    if (array_contains_nulls(DatumGetArrayTypeP(PG_GETARG_DATUM(0)))) ereport(ERROR, (errcode(ERRCODE_ARRAY_ELEMENT_ERROR), errmsg("array_contains_nulls")));
    ...
    if (!PG_ARGISNULL(1)) {
        ...
        if (filename_nelemsp != name_nelemsp) ereport(ERROR, (errmsg("filename_nelemsp != name_nelemsp")));
    }
    if (!PG_ARGISNULL(2)) {
