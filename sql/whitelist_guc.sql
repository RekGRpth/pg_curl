...
-- pg_curl has no predefined-role gate of its own (unlike pg_htmldoc's
-- pg_read_server_files/pg_execute_server_program), so
-- pg_whitelist_check_url()/check_local() treat a plain superuser() result
-- as "privileged": a superuser stays fully unrestricted while
-- pg_curl.whitelist is unset. PERFORM inside a plain DO block (no
-- EXCEPTION clause) is used to assert "not denied" without depending on
-- whether the request itself actually succeeds -- curl_easy_perform()
-- never raises on its own for a connection failure, so the DO block only
-- fails here if pg_whitelist_check_url() itself raises.
DO $$
BEGIN
...
-- Role names can't start with "pg_" (reserved). curl_test_none needs
-- LOGIN: the pg_curl.whitelist tests below need to \c into it directly,
-- since ALTER ROLE ... SET only takes effect for a new connection as that
-- role, not retroactively via SET ROLE in an already-open session.
